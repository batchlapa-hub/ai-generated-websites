...
document.addEventListener('DOMContentLoaded', function () {
  // Enhancement: Single-open-at-a-time behavior for FAQ items
    ...
    if (summary) {
      summary.addEventListener('click', function () {
        ...
        faqItems.forEach(otherItem => {
          ...
          if (otherSummary !== summary && otherItem.open) {
            otherItem.close();
          }
        });
      });
  ...
  // Enhancement: Simple autoplay/rotation for Testimonial Grid (if 3+ items exist)
  ...
  if (testimonialGrid && testimonialGrid.children.length >= 3) {
    ...
    function fadeIn() {
      ...
      const newItem = testimonials[currentIndex];
      ...
      }, 200); // Wait for fade out to start
    }

    // Start rotation every 5 seconds
    setInterval(fadeIn, 5000);
    
    // Trigger initial fade in after a short delay
