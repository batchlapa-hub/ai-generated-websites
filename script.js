...
document.addEventListener('DOMContentLoaded', () => {
  ...
  faqItems.forEach(item => {
    item.addEventListener('click', function() {
      const currentActive = this.querySelector('summary')?.parentElement;
      
      if (currentActive && currentActive !== this) {
        currentActive.classList.remove('active');
        currentActive.querySelector('.faq-content').style.maxHeight = null;
      }
  ...
  if (testimonialGrid && testimonialGrid.children.length >= 3) {
    ...
    setInterval(() => {
      ...
      setTimeout(() => {
        ...
        testimonials.forEach(t => t.style.opacity = '');
        
        // Move index
        currentIndex = (currentIndex + 1) % testimonials.length;
        
        // Fade in new
        testimonials[currentIndex].style.opacity = '0.8';
